...
try:
    ...
    import utils.triggers as triggers, utils.effects as effects, utils.cards as cards
    from utils.cards import Card
    from utils.effects import Effect
# except:
...
class Game:
    ...
    def __str__(self) -> str:
        ...
        str_ += cards.print_cards(self.hand, card_per_line=3, max_symbols_per_line=50) if len(self.hand) > 0 else ""
        return str_
    ...
    def check_playable(self, card_idx):
        ...
        if triggers.check_trigger_start_turn(card.playableTrigger, self) == False:
            return False
    ...
    def play_card(self,card_idx):
        ...
        if self.search_effect_play_count_buff:
            effects.effect_roll(playEffects, self)
        ...
        if self.card_create_search:
            if self.plan == "ProducePlanType_Plan2":
                new_card = random.choice(cards.all_logic_upgraded_ssr_cards)
            else:
                new_card = random.choice(cards.all_sense_upgraded_ssr_cards)
